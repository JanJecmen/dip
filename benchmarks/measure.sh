...
for run in $(seq 1 $RUNS); do
    ...
    for i in $BENCH; do
        ...
        R_ENABLE_JIT=0 timeout $TIMEOUT ${PLAIN_R}  -e \
            "{options(warn=-1); setwd('$D'); source('$i'); system.time(execute())[[3]]; t <- c(); for (i in 1:12) t <- c(t, system.time(execute())[[3]]); for (i in 5:12) write(paste0('1 R_ENABLE_JIT=0 vanilla-r, $T, ', t[[i]]), stderr())}" \
            > /dev/null 2>>$log || echo "" >>$log

        R_ENABLE_JIT=2 timeout $TIMEOUT ${PLAIN_R}  -e \
            "{options(warn=-1); setwd('$D'); source('$i'); system.time(execute())[[3]]; t <- c(); for (i in 1:12) t <- c(t, system.time(execute())[[3]]); for (i in 5:12) write(paste0('2 R_ENABLE_JIT=2 vanilla-r, $T, ', t[[i]]), stderr())}" \
            > /dev/null 2>>$log || echo "" >>$log

        R_ENABLE_JIT=2 timeout $TIMEOUT ${SCRIPTPATH}/R  -e \
            "{options(warn=-1); setwd('$D'); source('$i'); system.time(execute())[[3]]; t <- c(); for (i in 1:12) t <- c(t, system.time(execute())[[3]]); for (i in 5:12) write(paste0('3 R_ENABLE_JIT=2 rir, $T, ', t[[i]]), stderr())}" \
            > /dev/null 2>>$log || echo "" >>$log
